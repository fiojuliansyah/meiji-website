...
<div class="page-wrapper">
   <div class="page-content">
       ...
       <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
           <div class="breadcrumb-title pe-3">{{ translate('Edit') }} {{ translate('RandD') }}</div>
       </div>
       <!--end breadcrumb-->
       <div class="card">
           <div class="card-body">
               <form action="{{ route('randds.update', ['lang' => app()->getLocale(), 'randd' => $randd->id]) }}" method="POST">
                       ...
                       <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="lang-{{ $language->code }}"
                           ...
                           <div class="mb-3">
                               ...
                               <input class="form-control" type="text" name="translations[{{ $language->code }}][title]"
                                   id="title_{{ $language->code }}" value="{{ $randd->getTranslation('title', $language->code, false) }}" required>
                           </div>
                           <div class="mb-3">
                               ...
                               <textarea class="editor" name="translations[{{ $language->code }}][content]"
                                   id="content_{{ $language->code }}" required>{{ $randd->getTranslation('content', $language->code, false) }}</textarea>
                           </div>
                       </div>
                       @endforeach
                   </div>
                   ...
                   <!-- Submit Button -->
                   <div class="col-md-12">
                       <button type="submit" class="btn btn-primary">{{ translate('Update') }} {{ translate('RandD') }}</button>
                   </div>
               </form>
           </div>
       </div>
