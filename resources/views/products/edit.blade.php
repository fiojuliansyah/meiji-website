    ...
    <div class="page-header">
        <div class="page-header-left d-flex align-items-center">
            <div class="page-header-title">
                <h5 class="m-b-10">{{ translate('Edit') }} {{ translate('Product') }}</h5>
            </div>
        </div>
            ...
            <div class="card-body">
                <form action="{{ route('products.update', ['lang' => app()->getLocale(), 'product' => $product->id]) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" 
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" 
                                    ...
                                    class="form-control"
                                    value="{{ $product->getTranslation('name', $language->code) }}"
                                    required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">
                                ...
                                <textarea class="editor"
                                    ...
                                    required>{{ $product->getTranslation('content', $language->code) }}</textarea>
                            </div>
                        </div>
                        @endforeach
                    </div>
                    <div class="mb-3">
                        ...
                        <input class="form-control" type="date" name="date_published" value="{{ $product->date_published }}">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">
                            End Date
                        </label>
                        <input class="form-control" type="date" name="end_date" value="{{ $product->end_date }}">
                    </div>
                    ...
                    <button type="submit" class="btn btn-primary">
                        {{ translate('Update') }} {{ translate('Product') }}
                    </button>
                </form>
            </div>
        </div>
