...
<section class="single-product {{ isset($product->category->{'is-validate'}) ? 'bg-blur' : '' }}" id="single-product">
  ...
  <section class="shop shop-2 {{ isset($product->category->{'is-validate'}) ? 'bg-blur' : '' }}" id="related-products"> 
    <div class="container"> 
      <div class="row"> 
        <div class="col-12"> 
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
      // Check if the modal element exists (meaning validation is required)
      const disclaimerModal = document.getElementById('disclaimerModal');
      
      if (disclaimerModal) {
          // Initialize modal
          var modal = new bootstrap.Modal(disclaimerModal, {
              backdrop: 'static',
              keyboard: false
          });
          
          // Check if user has already agreed before
          var hasAgreed = localStorage.getItem('disclaimerAgreed_{{ $product->category->id }}');
          
          if (!hasAgreed) {
              // If user hasn't agreed yet, show the modal
              modal.show();
          } else {
              // If user has already agreed, remove blur immediately
              document.getElementById('single-product').classList.remove('bg-blur');
              const relatedProducts = document.getElementById('related-products');
              if (relatedProducts) {
                  relatedProducts.classList.remove('bg-blur');
              }
          }
